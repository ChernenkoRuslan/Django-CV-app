...
class ModelDetected:
    ...
    def load_video(self, filename: str, input_file: str, output_file: str):
        ...
        # -------------------------------------------------------
        ### Video output ####
        self.VIDEO_CODEC = "H264"  # изначально тут был mp4, поменяю на H264
        if self.verbose:
    ...
    def update_tracking(self, centers_old, obj_center, thr_centers, lastKey, frame, frame_max):
        ...
        lastpos = [(i[0], i[2])
                   for i in lastpos if abs(i[1] - frame) <= frame_max]
    ...
    def canny_edges(self, image):
        ...
        ex, threshold = cv2.threshold(
            image, 1, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
        # print(threshold)
    ...
    def detected(self):
        ...
        for i in range(int(self.video.get(cv2.CAP_PROP_FRAME_COUNT))):
            ...
            if len(positions_frame) > 0:
                positions_frame = positions_frame.sort_values(
            ...
            for ix, row in enumerate(positions_frame.iterrows()):
                ...
                xmin, ymin, xmax, ymax, confidence, category, = row[1].astype(
                    'int')
                ...
                cv2.rectangle(ROI, (xmin, ymin), (xmax, ymax),
                              (0, 0, 255), 2)  # box
                for center_x, center_y in centers_old[id_obj].values():
                    cv2.circle(ROI, (center_x, center_y), 5,
                               (0, 0, 255), -1)  # center of box
            ...
            # Рисуем трапецию
            cv2.polylines(overlay, pts=self.area_roi,
                          isClosed=True, color=(0, 255, 0), thickness=2)
            cv2.fillPoly(overlay, self.area_roi, (0, 255, 0))
            frame = cv2.addWeighted(
                overlay, self.alpha, frame, 1 - self.alpha, 0)
    ...
    def create_history_df(self, history):
        self.history_df = pd.DataFrame.from_dict(history).T.rename(
            columns={0: "time", 1: 'accuracy', 2: 'class'})
        ...
        self.history_df['class'] = self.history_df['class'].apply(
            lambda x: self.dict_classes[x])
        self.history_df['accuracy'] = self.history_df['accuracy'].apply(
            lambda x: round(x, 2))
...
if __name__ == "__main__":
    ...
    detector = ModelDetected(model=model, type_cam='front', skip_frames=10,
                             conf_level=0.7, verbose=False, dict_classes=dict_classes, device=device)
